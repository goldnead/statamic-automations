...
/**
 * Unpublishes an existing Statamic entry, identified by id (token-resolved).
 *
 * Side effects are gated behind `persist_statamic_changes` so test runs
 * never write.
 */
class UnpublishEntryAction implements AutomationAction
{
    public static function handle(): string
    {
        return 'unpublish_entry';
    }
    ...
    {
        return 'Sets an existing entry to draft (unpublished).';
    }
    ...
    public function execute(AutomationContext $context, array $config): ActionResult
    {
        ...
        if ($context->isTestMode() && ! config('automations.test_mode.persist_statamic_changes', false)) {
            return ActionResult::success([
                'preview' => ['entry_id' => $id, 'published' => false],
                'note' => 'Test mode — entry not unpublished.',
            ]);
        }
        ...
        return ActionResult::success([
            'entry' => ['id' => $entry->id(), 'slug' => $entry->slug(), 'published' => false],
        ]);
    }
}
