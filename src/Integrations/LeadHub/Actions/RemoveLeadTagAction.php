...
class RemoveLeadTagAction implements AutomationAction
{
    public function __construct(protected LeadHubAdapter $adapter) {}

    public static function handle(): string
    {
        return 'leadhub.remove_tag';
    }

    public static function label(): string
    {
        return 'Remove Lead Tag';
    }

    public static function description(): ?string
    {
        return 'Removes a tag from a LeadHub lead.';
    }
    ...
    public function execute(AutomationContext $context, array $config): ActionResult
    {
        ...
        if ($context->isTestMode() && ! config('automations.test_mode.persist_leadhub_changes', false)) {
            return ActionResult::success([
                ...
                'note' => 'Test mode — LeadHub tag removal skipped.',
            ]);
        }
        ...
        $result = $this->adapter->removeTag((string) $leadId, (string) $tag);

        return $result['ok']
            ? ActionResult::success(['lead_id' => $leadId, 'tag' => $tag])
            : ActionResult::failed($result['error'] ?? 'LeadHub remove-tag failed.');
    }
}
